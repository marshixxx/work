...
class VkUser:
    """Kласс, предоставляющий доступ к информации через API VK с помощью токена пользователя."""
    def get_params(self, add_params: dict = None):
        """Класс, предоставляющий доступ к параметрам методов API VK.
    ...
    def get_user_name(self, user_id):
        """Класс, возвращающий имя и фамилию пользователя VK.
    ...
    def get_top_photos(self, partner_id):
        """Класс, возвращающий список из нескольких ссылок на самые популярные фотографии пользователя.
    ...
    def find_partner(self):
        """Метод для поиска потенциальных партнёров в VK и добавления их в базу данных.
...
class VkBot:
    """Класс, использующий токен группы для взаимодействия с пользователем."""
    def __init__(self):
        self.commands = ['привет, прив, hi, hello, здравствуйте',
                         'старт, начать, го, go, start',
                         'пока, bye, до свидания, выход']
    ...
    def write_msg(self, user_id, message, attachment='', **kwargs):
        ...
        :type user_id: int
        :param message: Текст сообщения, отправляемый пользователю VK.
        :type message: str
    ...
    def chat_keyboard(self, buttons, button_colors):
        """Класс, использующий токен бота, для создания новой клавиатуры.
    ...
    def offer_partner(self, partner_id, first_name, last_name):
        """Класс, использующий токен группы, для отправки сообщения пользователю VK с его фото и предложением о партнерстве.
    ...
    def processing_messages(self, user_id, message_text):
        ...
        Реализован многоуровневый бот, который может обработать сообщения сразу от нескольких пользователей,
        используя библиотеку threading. У каждого пользователя будет свой тред и его текущее положение в базе данных,
        а обработка сообщения зависит от его состояния в базе данных.
        После обработки сообщения на новое, выполнил необходимые инструкции.
